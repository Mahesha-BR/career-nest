        ...
        <Card padding={ 0 }>
            <div className='flex gap-3 p-4'>
                <Link to={`/profile/${item?.user?._id}`} className='w-12 h-12 rounded-4xl'>
                    <img className='rounded-4xl w-12 h-12 border-white cursor-pointer'
            ...
            </div>
            {
                <div className='text-md p-4 my-4 whitespace-pre-line flex-grow'>
                    ...
                    { desc?.length < 50 ? null : <span onClick={ () => setSeeMore( prev => !prev ) }
                        className='cursor-pointer text-blue-600'>{ seeMore ? "See Less" : "See More" }</span> }
                </div>
            }
            {
            ...
            {/*setComment( true ) */ }
            {
                !profile && <div className='flex p-1'>
                    <div onClick={ handleLikeFunc } className='w-[33%] justify-center flex gap-2 items-center border-gray-100 p-2 cursor-pointer hover:bg-gray-100'>
                        { liked ? <ThumbUpOutlinedIcon sx={ { fontSize: 22, color: "blue" } } />
                            : <ThumbUpOutlinedIcon sx={ { fontSize: 22 } } /> }
                        <span>{ liked ? 'Liked' : "Like" }</span></div>
                    <div onClick={ handleCommentBoxOpenClose }
                        className='w-[33%] justify-center flex gap-2 items-center border-gray-100 p-2 cursor-pointer hover:bg-gray-100'>
                        <CommentIcon sx={ { fontSize: 22 } } /> <span>Comment</span></div>
                    <div onClick={copyToClipboard}
                    className='w-[33%] justify-center flex gap-2 items-center border-gray-100 p-2 cursor-pointer hover:bg-gray-100'>
                        <SendIcon sx={ { fontSize: 22 } } /> <span>Share</span></div>
                </div>
            }
            {/*comment section */ }
            {
                comment && <div className='p-4 w-full'>
                    <div className='flex gap-2 items-center'>   
                        ...
                        <form className="w-full flex gap-2" onSubmit={ handleSendComment }>
                            ...
                            placeholder='Add a comment....' className='w-full border-1 py-3 px-5 rounded-3xl hover:bg-gray-100' />
